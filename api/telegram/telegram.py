...
class Telegram(object):
    ...
    def get_file(self, data: dict):
        r = self.request.get(url=f'https://api.telegram.org/bot{settings.BOT_TOKEN}/getFile?file_id={data.get("file_id")}')
        return {
            'status': r.ok,
            'chat_id': data.get('chat_id'),
    ...
    def download_file(self, data: dict = Depends(filter_data)):
        r = self.request.get(url=f'https://api.telegram.org/file/bot{settings.BOT_TOKEN}/{data.get("file_path")}')
        return {
            'status': r.ok,
            'chat_id': data.get('chat_id'),
            'file': r.raw
